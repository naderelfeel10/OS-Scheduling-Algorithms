...
#define DEFAULT_USER_BURST_TIME 20
...
{
	register struct schedproc *rmp ,*temp;
	int rv, proc_nr_n;
	...
	rmp = &schedproc[proc_nr_n];
         if(rmp->priority >= MAX_USER_Q && rmp->priority <= MIN_USER_Q)
         {
           ...
           {
             temp = &schedproc[i];
             if( temp->flags == IN_USE ){
              if( temp->BRUST_TIME < MinBurstTime || i == 0)
                    {
                       MinBurstTime = temp->BRUST_TIME;
                       temp = &schedproc[i];
...
int do_start_scheduling(message *m_ptr)
{
	...
	rmp->max_priority = (unsigned) m_ptr->SCHEDULING_MAXPRIO;
        rmp->BRUST_TIME = DEFAULT_USER_BURST_TIME;
